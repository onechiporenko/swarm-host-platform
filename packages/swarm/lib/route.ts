...
import { Lair, CRUDOptions } from 'lair-db';
import methods = require('methods');
import { assert } from './utils';

function defaultNext(req: Request, res: Response, data: object[] | object | any): Response {
  return res.json(data);
}

function defaultHandler(req: Request, res: Response, next: NextFunction, lair: Lair): Response {
  return res.json({});
}

export type Handler = (req: Request, res: Response, next: NextFunction, lair: Lair) => any;
export type CustomNext = (req: Request, res: Response, data: object[] | object | any, lair: Lair) => any;
...
export default class Route {
  ...
  public static get(path: string, modelName: string, lairOptions: CRUDOptions = {}, customNext: CustomNext = defaultNext): Route {
    const handler = (req: Request, res: Response, next: NextFunction, lair: Lair) => {
  ...
  public static post(path: string, modelName: string, lairOptions: CRUDOptions = {}, customNext: CustomNext = defaultNext): Route {
    return Route.createRoute('post', path, (req: Request, res: Response, next: NextFunction, lair: Lair) => {
  ...
  public static put(path: string, modelName: string, lairOptions: CRUDOptions = {}, customNext: CustomNext = defaultNext): Route {
    return Route.createRoute('put', path, (req: Request, res: Response, next: NextFunction, lair: Lair) => {
      const parameters = Object.keys(req.params);
      const id = parameters.length === 1 ? req.params[parameters[0]] : undefined;
  ...
  public static patch(path: string, modelName: string, lairOptions: CRUDOptions = {}, customNext: CustomNext = defaultNext): Route {
    return Route.createRoute('patch', path, (req: Request, res: Response, next: NextFunction, lair: Lair) => {
      const parameters = Object.keys(req.params);
      const id = parameters.length === 1 ? req.params[parameters[0]] : undefined;
      assert('identifier is not provided', !!id);
      const result = lair.updateOne(modelName, id, req.body, lairOptions);
