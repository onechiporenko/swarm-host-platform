...
export function printRoutesMap(path: string[], layer: any): void {
  if (layer.route) {
    layer.route.stack.forEach(
      printRoutesMap.bind(null, path.concat(split(layer.route.path)))
    );
  } else if (layer.name === 'router' && layer.handle.stack) {
    layer.handle.stack.forEach(
      printRoutesMap.bind(null, path.concat(split(layer.regexp)))
    );
  } else if (layer.method) {
    logger.info({
      level: 'info',
      message: `${layer.method.toUpperCase()} ${path
